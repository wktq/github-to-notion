...
if (process.argv.length < 4) {
  console.error('Usage: node import-to-notion-resume.js <project-file> <database-id> <start-index>')
  process.exit(1)
}
...
// Process items starting from the specified index
const itemsToProcess = project.items.nodes.slice(startIndex)
...
for (const item of itemsToProcess) {
  ...
  if (result) {
    ...
    failedItems.push({
      ...
      index: startIndex + processedCount + failedCount - 1
    })
  }
  
  // Show progress
  if ((processedCount + failedCount) % 10 === 0) {
    console.log(`Progress: ${processedCount + failedCount}/${itemsToProcess.length} (${processedCount} succeeded, ${failedCount} failed)`)
  }
...
console.log(`Total processed from beginning: ${startIndex + processedCount + failedCount} items`)

if (failedItems.length > 0) {
  console.log('\nFailed items:')
  failedItems.forEach(item => {
    console.log(`- ${item.title} (index: ${item.index}, created: ${item.createdAt})`)
  })
  
  // Save failed items to a file for retry
  const existingFailed = await fs.readFile('failed-items.json', 'utf-8').catch(() => '[]')
  const allFailed = [...JSON.parse(existingFailed), ...failedItems]
  await fs.writeFile('failed-items.json', JSON.stringify(allFailed, null, 2))
  console.log('\nFailed items appended to failed-items.json')
}